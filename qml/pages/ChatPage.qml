...
Page {
    ...
    Connections {
        target: MediaCache
        onDownloadFinished: {
            if (!chatPage.openingAttachment || key !== chatPage.pendingOpenKey)
                return
            chatPage.openingAttachment = false
            chatPage.pendingOpenKey = ""
            if (fileUrl)
        ...
        onDownloadFailed: {
            ...
            chatPage.openingAttachment = false
            chatPage.pendingOpenKey = ""
            chatPage.errorText = error || i18n.tr("Couldn't open attachment")
        }
    }
    ...
    ListView {
        ...
        delegate: MessageDelegate {
            ...
            onFileOpenRequested: chatPage.openFileAttachment(fileInfo)
        ...
        ActionSelectionPopover {
            actions: ActionList {
                Action {
                    iconName: "image"
                    text: i18n.tr("Photos…")
                    onTriggered: chatPage.openContentHub(ContentType.Pictures)
                }
                Action {
                    iconName: "media-record"
                    text: i18n.tr("Record video…")
                    onTriggered: chatPage.openContentHub(ContentType.Videos)
                }
                Action {
                    iconName: "audio-input-microphone"
                    text: i18n.tr("Record audio…")
                    onTriggered: chatPage.openContentHub(ContentType.Music)
                }
                Action {
                    iconName: "document-open"
                    text: i18n.tr("Browse files…")
                    onTriggered: chatPage.openContentHub(ContentType.Documents)
                }
            }
        }
    }
